...
export class PitchDetector {
    ...
    isValidDetection(frequency, clarity) {
        if (!frequency || frequency <= 0)
            return false;
        if (!clarity || clarity < this.config.minClarity)
            return false;
        if (frequency < this.config.minFrequency)
            return false;
        if (frequency > this.config.maxFrequency)
            return false;
        return true;
    }
    /**
 ...
 */
export function detectFrequency(buffer, sampleRate = 44100, minClarity = 0.75) {
...
//# sourceMappingURL=pitch-detector.js.map